...
export function parseCandidate(candidate) {
    if (candidate.votes) {
        candidate.votes = LemoUtils.moToLemo(candidate.votes).toString(10)
    }
...
export function parseTxRes(res) {
    ...
    tx.gasUsed = parseNumber(res.tx.gasUsed)
...
export function parseTxData(type, data) {
    ...
    switch (type) {
        ...
        case TxType.VOTE:
            break
        case TxType.BOX_TX:
            data = JSON.parse(LemoUtils.toBuffer(data).toString())
...
export function parseMoney(str) {
    ...
    Object.defineProperty(result, 'toMoney', {
        enumerable: false,
        value: LemoUtils.formatLEMO.bind(null, result),
    })
    return result
}
