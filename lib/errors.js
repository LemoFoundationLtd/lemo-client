export default {
    ...
    invalidConnConfig: config => `invalid conn config: ${JSON.stringify(config)}`,
    ...
    InvalidTxTimeOut: () => 'The transaction has expired',
    InvalidHeight: () => 'The height must be number',
    InvalidWatchId: () => 'stopWatchBlock needs a parameter which is returned by watchBlock',
    InvalidWatchBlockHeight: (lastHeight, newHeight) => `new block height [${newHeight}] must be bigger than the height [${lastHeight}] of current block`,
    InvalidWatchTxConfig: (filterTxConfig) => `filterTxConfig is invalid: ${JSON.stringify(filterTxConfig)}`,
}
