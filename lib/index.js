...
class LemoClient {
    constructor(config = {}) {
        ...
        this.parser = parser
        this.requester = new Requester(newConn(this.config.conn), this.config.requester)
        this.blockWatcher = new BlockWatcher(this.requester, this.config.startWatchHeight)
        this.txWatcher = new TxWatcher(this.requester, this.blockWatcher, {
            serverMode: this.config.serverMode,
            txPollTimeout: this.config.httpTimeOut,
        })
     ...
     */
    createAPI(moduleName, apiName, methodNameOrFunc) {
        ...
        if (typeof methodNameOrFunc === 'function') {
            ...
            apiConfig.call = (...args) => {
                return this.requester.send(methodNameOrFunc, args)
            }
        } else {
            throw new Error(errors.InvalidAPIName(methodNameOrFunc))
        }
        newApiAndAttach(this, moduleName, apiConfig)
    }
}
