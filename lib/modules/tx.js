...
import ModifySignersTx from '../tx/special_tx/modify_signers_tx'
import BoxTx from '../tx/special_tx/box_tx'
import ContractCreationTx from '../tx/special_tx/contract_creation_tx'
...
const apis = {
     ...
     * @param {string} address Account address
     * @param {string} assetId asset transactions' hash
     * @param {number} index Index of transactions
     * @param {number} limit The count of transactions required
     * @return {Promise<object>}
     */
    async getAssetTxList(address, assetId, index, limit) {
        const result = await this.requester.send(`${TX_NAME}_getAssetTxList`, [address, assetId, index, limit])
        if (!result) {
            return null
        }
        return this.parser.parseTxListRes(result)
     ...
     * @return {string}
     */
    signReimbursement(privateKey, noGasTxStr, gasPrice, gasLimit) {
     ...
     * @return {string}
     */
    signCreateTempAddress(privateKey, txConfig, userId) {
        let tempConfig = {
            ...txConfig,
        ...
        const tx = new ModifySignersTx(tempConfig, modifySignersInfo)
        tx.signWith(privateKey)
        return JSON.stringify(tx.toJson())
    },
    /**
     ...
     * @return {string}
     */
    signModifySigners(privateKey, txConfig, signers) {
        txConfig = checkChainID(txConfig, this.chainID)
        const modifySignersInfo = {
            signers,
        }
        const tx = new ModifySignersTx(txConfig, modifySignersInfo)
        tx.signWith(privateKey)
        return JSON.stringify(tx.toJson())
    },
    /**
     ...
     * @return {string}
     */
    signBoxTx(privateKey, txConfig, subTxList) {
        txConfig = checkChainID(txConfig, this.chainID)
     ...
     * @return {string}
     */
    signContractCreation(privateKey, txConfig, codeHex, constructorArgsHex) {
        txConfig = checkChainID(txConfig, this.chainID)
