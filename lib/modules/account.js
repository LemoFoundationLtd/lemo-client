...
const apis = {
     ...
     * @return {Promise<object>}
     */
    getEquityList(address, index, limit) {
        if (!isLemoAddress(address)) {
            throw new Error(error.InvalidAddress(address))
        }
        return this.requester.send(`${ACCOUNT_NAME}_getEquityList`, [address, index, limit])
    },
    /**
     ...
     * @return {Promise<object>}
     */
    getEquityListByAssetCode(address, assetCode, index, limit) {
        if (!isLemoAddress(address)) {
            throw new Error(error.InvalidAddress(address))
        }
        return this.requester.send(`${ACCOUNT_NAME}_getEquityListByAssetCode`, [address, assetCode, index, limit])
    },
    /**
     * 获取指定资产类型的发行信息
